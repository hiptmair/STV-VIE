function [A0, A1, T] = build_STFb(mesh, k0, k, aO, a, grad_a, tol)
...
comm_Gxy = @(X,Y) (1 ./ a(Y) - 1./a(X)) .* femGreenKernel(X, Y, '[H0(kr)]', k0);
...
comm_dyGxy{1} = @(X,Y) a(X) .* (1./a(Y) - 1./a(X)) .* femGreenKernel(X, Y, 'grady[H0(kr)]1', k0);
comm_dyGxy{2} = @(X,Y) a(X) .* (1./a(Y) - 1./a(X)) .* femGreenKernel(X, Y, 'grady[H0(kr)]2', k0);
comm_dyGxy{3} = @(X,Y) a(X) .* (1./a(Y) - 1./a(X)) .* femGreenKernel(X, Y, 'grady[H0(kr)]3', k0);
