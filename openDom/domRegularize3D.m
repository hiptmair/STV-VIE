...
%         norm(gradrRm1(:,:,3))                               %%% DEBUG %%%
        ...
        elseif strcmp(v.typ,'P1')
            ...
            for j = 1:Nbas
                ...
                elseif strcmp(green,'[log(r)]') && strcmp(v.opr,'grad[psi]')
                    NUj     = NUel(j,:);
                    V{1}(:,j) = NUj(1)/hj .* logR;
                    V{2}(:,j) = NUj(2)/hj .* logR;
                    V{3}(:,j) = NUj(3)/hj .* logR;
                ...
                elseif strcmp(green,'[log(r)]') && strcmp(v.opr(1:end-1),'grad[psi]')
                    jj     = str2double(v.opr(end));
                    NUj     = NUel(j,:);
                    V(:,j) = NUj(jj)/hj .* logR;
                ...
                % Ignacio: This computes int grad(G) x grad(psi), 
                elseif strcmp(green,'grady[log(r)]') && strcmp(v.opr,'xgrad[psi]')
                    
                    
                    % NUel : normal to edges
                    gradxgrad = cross(gradlogR, Xun * NUel(j, :) );
%                     V{1}(:,j) = gradxgrad(:, 1);
%                     V{2}(:,j) = gradxgrad(:, 2);
                    V(:,j) = gradxgrad(:, 3);
                                    
                elseif strcmp(green,'grady[log(r)]') && strcmp(v.opr,'grad[psi]')
                    
                    V(:, j) = gradlogR*NUel(j,:)'/hj;
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
